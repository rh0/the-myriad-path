...
I know the descriptions of this behavior sounds a little complex.  It really isn't, the behaviour is pretty intuative once you play with it for a bit (and I encourage you to do so, if you haven't already.)  However, though the rules for the behaviour are fairly simple, it can lead to some very complex behaviours.  Take care to keep your stylesheets organized and as "clean" as possible to avoid falling into pits of eternal darkness as you hunt for what decleration is overriding the one you just created.  For example there is rarely a reason to have two `p` declerations in your CSS as I do in my last example. It only serves to confuse.  These should be combined into one,
```
p {
  color: blue;
  text-align: right;
...
```
This way when you need to modify `p` in the future, there is only one place to look and change.  If you are curious or would like a more thorough explination of these behaviors https://developer.mozilla.org/en-US/docs/Web/Guide/CSS/Getting_started/Cascading_and_inheritance[the mozilla spec] does a much better job than I am able.
====
